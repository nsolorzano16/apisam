...
namespace apisam.repos
{
    public class PacientesRepo : IPaciente
    {
        ...
        public bool AddPaciente(Paciente paciente)
        {
            ...
            var pacienteBuscado = _db.Select<Paciente>().FirstOrDefault(x =>
            x.Identificacion == paciente.Identificacion);
            ...
            GetPacientes(int pageNo, int limit, string filter)
        {
            ...
            var _qry2 = _qry;
            _qry += " ORDER BY u.PacienteId DESC";
            ...
            if (limit > 0)
            {
                _response.TotalItems =
                    _db.Select<Paciente>(_qry2).ToList().Count();
